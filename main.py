    ...
    def routeDistance(self):
        if self.distance == 0:
            ...
            for i in range(0, len(self.route)):
                fromity = self.route[i]
                toCity = None
                if i + 1 < len(self.route):
                    toCity = self.route[i+1]
                else:
                    toCity = self.route[0]
                pathDistance += fromity.distance(toCity)
...
#geneticAlgorithmPlot(population=cityList, popSize=popSize, eliteSize=eliteSize, mutationRate=mutationRate, generations=generations)
